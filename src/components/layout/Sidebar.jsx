...
const ThemeToggle = ({ isDark, onToggle }) => (
  <button onClick={onToggle} aria-label="Basculer le thème"
    style={{
      ...
      transition:"background 0.3s ease", display:"flex", alignItems:"center", padding:3,
    }}>
    <div style={{
      ...
      {isDark ? (
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
      ) : (
...
const Sidebar = ({ currentUser, activePage, setActivePage, sidebarCollapsed, setSidebarCollapsed, isDark, setIsDark, badgeCounts, onLogout }) => {
  const COLORS = useTheme();

  return (
    <aside style={{
      width: sidebarCollapsed ? 68 : 240, background: COLORS.surface,
      ...
      {/* Logo */}
      <div style={{ padding: sidebarCollapsed ? "20px 14px" : "20px 24px", borderBottom:`1px solid ${COLORS.border}`, display:"flex", alignItems:"center", gap:12, cursor:"pointer", transition:"border-color 0.35s" }}
      ...
      {/* Theme toggle */}
      <div style={{ padding:"12px 16px", borderTop:`1px solid ${COLORS.border}`, display:"flex", alignItems:"center", justifyContent: sidebarCollapsed ? "center" : "flex-start", gap:10 }}>
        <ThemeToggle isDark={isDark} onToggle={() => setIsDark(d => !d)} />
        {!sidebarCollapsed && <span style={{ fontSize:12, color:COLORS.textMuted }}>{isDark ? "Mode sombre" : "Mode clair"}</span>}
      </div>
      ...
      {!sidebarCollapsed && (
        <div style={{ padding:"12px 20px", borderTop:`1px solid ${COLORS.border}` }}>
          ...
          <button onClick={onLogout} aria-label="Se déconnecter"
            style={{ width:"100%", padding:"6px 12px", borderRadius:8, border:`1px solid ${COLORS.border}`, background:"transparent", color:COLORS.textMuted, fontSize:11, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:6, transition:"all 0.15s" }}
